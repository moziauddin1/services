...
=== Major requirements:
 ...
 or enclosed taxa below this taxa. This identifier spans multiple versions of a tree.
...
systems. This is also used to generate the hash for taxon identifiers, so it is important that they remain consistent. So
if you copy a branch from one tree to another that taxon concept remains the same, even if you regenerate the hashes.
...
NOTE: tree_elements store links to instance, name and themselves to make them self contained. With the join table this
is complicated because a tree_element belongs to multiple versions and as such the element_link needs to be generated at
display time depending on the version being shown.
...
. Add aleaf element and assign the tree_version_element a new taxon ID
