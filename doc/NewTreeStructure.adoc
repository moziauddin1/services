...
| ~200MB (inc 77MB index)
|===

which makes the new tree structure smaller than the old one.
...
this would need a node ids field added to tree element.
...
The most appropriate solution is to use the mapper to map node URI to tree_elements.
