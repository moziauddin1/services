...
class TreeJsonEditController {
    ...
    private def dropNameOntoNode(Arrangement ws, Node focus, Node target, Name name, DropUrisOntoNodeParam param) {
        ...
        if (paramAction || (possibleActions.size() == 1 && !possibleActions.first().needsWarning)) {
            ...
            return [
                    success  : true,
                    focusPath: queryService.findPath(ws.node, newFocus).collect { Node n -> linkService.getPreferredLinkForObject(n) },
                    refetch  : result.modified.collect { Node n ->
                        queryService.findPath(newFocus, n).collect { Node n2 -> linkService.getPreferredLinkForObject(n2) }
                    }
            ]
        } else {
    ...
    def revertNode(RevertNodeParam param) {
        handleException {
            ...
            return render([
                    success  : true,
                    focusPath: target == focus ? queryService.findPath(ws.node, curr).collect { Node n -> linkService.getPreferredLinkForObject(n) } : null,
                    refetch  : [
    ...
    def removeNode(RemoveNodeParam param) {
        handleException {
            ...
            def focusPath = queryService.findPath(ws.node, newFocus).collect { Node n ->
                linkService.getPreferredLinkForObject(n)
            }
            def targetPath = queryService.findPath(newFocus, newCheckout).collect { Node n ->
                linkService.getPreferredLinkForObject(n)
            }
