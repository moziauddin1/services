...
SELECT
  ...
  CASE WHEN basionym_inst.id IS NOT NULL
    THEN 'http://id.biodiversity.org.au/instance/${namespace}/' || basionym_inst.cites_id :: TEXT
  ELSE
    CASE WHEN primary_inst.id IS NOT NULL
      THEN 'http://id.biodiversity.org.au/instance/${namespace}/' || primary_inst.id :: TEXT
    ELSE NULL END
  ...
  (SELECT name_element
   FROM find_rank(n.id, 120)
   ORDER BY sort_order ASC
   LIMIT 1)                                                                     AS "genericName",
  (SELECT name_element
   FROM find_rank(n.id, 190)
   ORDER BY sort_order ASC
   LIMIT 1)                                                                     AS "specificEpithet",
  (SELECT name_element
   FROM find_rank(n.id, 191)
   ORDER BY sort_order ASC
  ...
  'http://id.biodiversity.org.au/name/${namespace}/' || first_hybrid_parent.id  AS "firstHybridParentNameID",
  second_hybrid_parent.full_name                                                AS "secondHybridParentName",
  'http://id.biodiversity.org.au/name/${namespace}/' || second_hybrid_parent.id AS "secondHybridParentNameID",
...
FROM name n
  ...
  JOIN instance_type primary_it ON primary_it.id = primary_inst.instance_type_id AND primary_it.primary_instance = TRUE
...
(SELECT
   ...
   syn ->> 'concept_link'                                AS "taxonConceptID",
   syn ->> 'cites'                                       AS "nameAccordingTo",
   syn ->> 'cites_link'                                  AS "nameAccordingToID",
 ...
 FROM tree_version_element tve
   ...
   LEFT OUTER JOIN NAME secondHybridParent ON syn_name.second_parent_id = secondHybridParent.id AND syn_nt.hybrid
   ,
   (SELECT DISTINCT name_path
    FROM tree_version_element tve
      JOIN tree_element te ON tve.tree_element_id = te.id
 ...
 SELECT
   ...
   CASE WHEN acc_nt.autonym
     THEN NULL
   ELSE
     regexp_replace(substring(acc_name.full_name_html FROM '<authors>(.*)</authors>'), '<[^>]*>', '', 'g')
   END                                                           AS "scientificNameAuthorship",
   ...
   'http://id.biodiversity.org.au/reference/${namespace}/' || acc_ref.id AS "nameAccordingToID",
   profile -> 'APC Comment' ->> 'value'                          AS "taxonRemarks",
   profile -> 'APC Dist.' ->> 'value'                            AS "taxonDistribution",
   -- todo check this is ok for synonyms
   regexp_replace(tve.name_path, '/', '|', 'g')                  AS "higherClassification",
 ...
 FROM tree_version_element tve
   ...
   LEFT OUTER JOIN NAME secondHybridParent ON acc_name.second_parent_id = secondHybridParent.id AND acc_nt.hybrid
   ,
   (SELECT DISTINCT name_path
    FROM tree_version_element tve
      JOIN tree_element te ON tve.tree_element_id = te.id
