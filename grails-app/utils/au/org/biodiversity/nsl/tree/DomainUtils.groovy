...
class DomainUtils {
    ...
    static Uri getNamespaceUri(UriNs ns) {
        ...
        if (isBlank(ns)) {
            ...
            return new Uri(getBoatreeNs(), 'voc')
        } else {
            // other namespaces are served up from here.
            return new Uri(ns, ns.label)
        }
    }
    ...
    static Map<Uri, Link> getProfileItemsAsMap(Node n) {
        ...
        getProfileItemsAsList(n).each { mm.put(getLinkTypeUri(it), it) }
        return mm
    }
    ...
    static Uri getRawLinkTypeUri(Link l) {
        if (l == null) return null
        return u(l.typeUriNsPart, l.typeUriIdPart)
    }
    ...
    static ValueNodeUri vnuForItem(Link l) {
        return l ? ValueNodeUri.findByRootAndLinkUriNsPartAndLinkUriIdPart(l.supernode.root, l.typeUriNsPart, l.typeUriIdPart) : null
    }
