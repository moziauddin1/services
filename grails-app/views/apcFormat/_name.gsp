<div class="apc name" id="${name.id}">
  ...
  <g:if test="${apcInstance}">
    ...
    <div data-nameId="${name.id}">
      <g:if test="${!excluded}">
        <accepted-name title='Accepted name'><st:preferedLink target="${primaryInstance ?: name}">${raw(name.fullNameHtml)}</st:preferedLink></accepted-name>
      </g:if>
      <g:else>
        <excluded-name title='excluded name'><st:preferedLink target="${primaryInstance ?: name}">${raw(name.fullNameHtml)}</st:preferedLink>
      ...
      | sensu
      <st:preferedLink target="${apcInstance?.reference}">${raw(apcInstance?.reference?.citationHtml)}</st:preferedLink>
      <a class="vertbar" href="${af.refAPCSearchLink(citation: apcInstance?.reference?.citation, product: params.product)}">
