<div class="apc name" id="${name.id}">
  <st:primaryInstance var="primaryInstance" name="${name}">
      ...
      <div data-nameId="${name.id}">
        <g:if test="${!excluded}">
          <accepted-name title='Accepted name'><a href="${preferredNameLink + '/api/apni-format'}">
            <st:encodeWithHTML text='${name.fullNameHtml}'/></a></accepted-name>
        </g:if>
        <g:else>
          <excluded-name title='excluded name'><a href="${preferredNameLink + '/api/apni-format'}">
            <st:encodeWithHTML text='${name.fullNameHtml}'/></a>
        ...
        | sensu
        <st:preferedLink
            target="${apcInstance?.reference}"><st:encodeWithHTML text='${apcInstance?.reference?.citationHtml}'/></st:preferedLink>
        ...
        <div class="well instances">
          ...
          <ul class="instance-notes list-unstyled">
            <af:getAPCNotes instance="${apcInstance}" var="instanceNote">
              <li>
                <instance-note><st:encodeWithHTML text='${instanceNote.value}'/></instance-note>
              </li>
            </af:getAPCNotes>
          </ul>
        </div>
        ...
        <af:sortedReferences instances="${misapplied}" var="synonym" sortOn="cites">
          <div data-nameId="${name.id}">
            <g:if test="${synonym.instanceType.misapplied}">
              <st:preferedLink target="${name}" api="api/apni-format"><st:encodeWithHTML text='${name.simpleNameHtml}'/></st:preferedLink>
            ...
            <accepted-name title='Accepted name'>
              <st:preferedLink target="${synonym.citedBy.name}"
                               api="api/apni-format"><st:encodeWithHTML text='${synonym.citedBy.name.fullNameHtml}'/></st:preferedLink>
    ...
    <g:elseif test="${synonymOf}">
      <af:sortedReferences instances="${synonymOf}" var="synonym" sortOn="cites">
        <div data-nameId="${name.id}">
          ...
          <g:if test="${synonym.instanceType.misapplied}">
            <st:preferedLink target="${name}" api="api/apni-format"><st:encodeWithHTML text='${name.fullNameHtml}'/></st:preferedLink>
            <st:preferedLink target="${primaryInstance ?: name}"><i title="Link to use in reference"
                                                                    class="fa fa-book"></i></st:preferedLink>
            sensu <st:encodeWithHTML text='${synonym.cites.reference.citationHtml}'/>
          </g:if>
          <g:else>
            <st:preferedLink target="${name}" api="api/apni-format"><st:encodeWithHTML text='${name.fullNameHtml}'/></st:preferedLink>
            <st:preferedLink target="${primaryInstance ?: name}"><i title="Link to use in reference"
    ...
    <g:else>
      <span class="text-muted"><st:encodeWithHTML text='${name.fullNameHtml}'/> not in APC. (Perhaps restrict your search to APC?)</span>
    </g:else>
  </st:primaryInstance>
</div>
