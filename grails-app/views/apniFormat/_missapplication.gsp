<af:sortedInstances instances="${instances}" var="instance">
  <misapplication>
    ...
    <st:preferredLink target="${instance.cites.name}" api="api/apni-format">${raw(instance.cites.name.fullNameHtml)}</st:preferredLink>
    <st:preferredLink target="${instance.cites}"><i title="Link to use in reference" class="fa fa-link hidden-print"></i></st:preferredLink>
